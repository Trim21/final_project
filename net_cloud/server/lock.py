...
class AioRedisReadWriteLock:
    ...
    lua_release_sha = None
    lua_extend_sha = None
    lua_reacquire_sha = None
    ...
    def __init__(self, redis: AioRedis, loop=None):
        self._redis = redis
        if loop:
            self._loop = loop
        else:
    ...
    async def register_script(self):
        cls = self.__class__
        client = self._redis

        if cls.lua_reacquire_sha is None:
            cls.lua_reacquire_sha = client.script_load(cls.LUA_REACQUIRE_SCRIPT)
        if cls.lua_release_sha is None:
            cls.lua_release_sha = client.script_load(cls.LUA_RELEASE_SCRIPT)
        if cls.lua_extend_sha is None:
            cls.lua_extend_sha = client.script_load(cls.LUA_EXTEND_SCRIPT)

    async def try_acquire(self, resource_name: str, timeout: int = 30):
        pass
